async function cwQuery() {
   ...
   main.ATD as "Actual Dept Date",
   sum(main.actl_wght) over (partition by main.mawb,   main.origin,   main.destination) as "actual weight",
   sum(case when (case when main.chrg_wght <= 0 then main.actl_wght else main.chrg_wght end ) < main.actl_wght then main.actl_wght else main.chrg_wght end) over (partition by main.mawb,   main.origin,   main.destination)
   ...
   'USD' as Currency,
   '' as "airline_rate",
   sum(coalesce(seccst.total_security_cost,0) + coalesce(fuelcst.total_fuel_cost,0) + coalesce(freightcst.total_freight_cost,0)) over (partition by main.mawb,   main.origin,   main.destination) as "Total Cost To Airline",
   sum(coalesce(fuelcst.total_fuel_cost,0)) over (partition by main.mawb,   main.origin,   main.destination) as "Total Fuel Surcharge",
   sum(coalesce(seccst.total_security_cost,0)) over (partition by main.mawb,   main.origin,   main.destination) as "Total Security Surcharge"
   from
